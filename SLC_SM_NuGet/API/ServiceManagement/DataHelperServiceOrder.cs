namespace Skyline.DataMiner.ProjectApi.ServiceManagement.API.ServiceManagement
{
	...
	public class DataHelperServiceOrder : DataHelper<Models.ServiceOrder>
	{
		...
		/// <inheritdoc />
		public override bool TryDelete(IEnumerable<Models.ServiceOrder> items)
		{
			if (items == null)
			{
				return true;
			...
			bool b = helper.TryDelete(lst.Where(i => i?.OrderItems != null).SelectMany(i => i.OrderItems).Where(i => i?.ServiceOrderItem != null).Select(i => i.ServiceOrderItem));
			b &= TryDelete(lst.Where(i => i != null).Select(i => i.ID));

			return b;
		}

		internal override List<Models.ServiceOrder> Read(IEnumerable<DomInstance> domInstances)
		{
