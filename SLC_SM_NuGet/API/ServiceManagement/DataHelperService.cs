namespace Skyline.DataMiner.ProjectApi.ServiceManagement.API.ServiceManagement
{
	...
	public class DataHelperService : DataHelper<Models.Service>
	{
		...
		/// <inheritdoc />
		public override bool TryDelete(IEnumerable<Models.Service> items)
		{
			if (items == null)
			{
				return true;
			...
			DataHelperServiceConfigurationValue dataHelperServiceConfigurationValue = new DataHelperServiceConfigurationValue(_connection);
			bool b = dataHelperServiceConfigurationValue.TryDelete(lst.Where(i => i?.Configurations != null).SelectMany(i => i.Configurations));
		...
		internal override List<Models.Service> Read(IEnumerable<DomInstance> domInstances)
		{
		...
		private List<Models.ServiceCategory> GetRequiredCategories(List<ServicesInstance> instances)
		{
			...
			{
				filter = filter.OR(ServiceCategoryExposers.Guid.Equal(guid));
			}
